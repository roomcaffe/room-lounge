    ...
    <>
      ...
      <a
        href={`https://wa.me/${wa}?text=${encodeURIComponent(
          "Përshëndetje Room Lounge Cafe, dua të rezervoj një tavolinë."
        )}`}
        target="_blank"
        rel="noreferrer"
        aria-label="WhatsApp"
        className={`fixed bottom-6 right-6 z-40 w-14 h-14 rounded-full flex items-center justify-center bg-[#25D366] text-white shadow-[0_10px_30px_rgba(37,211,102,0.45)] transition-all duration-500 hover:scale-110 ${
      ...
      <div
        className={`fixed bottom-0 inset-x-0 z-30 md:hidden p-4 bg-gradient-to-t from-[var(--bg)] via-[var(--bg)] to-transparent transition-all duration-500 ${
          show ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0 pointer-events-none"
        }`}
      >
        <Link href="/reservation" className="btn-gold w-full justify-center">
          Rezervo Tavolinë
        </Link>
      </div>
    </>
